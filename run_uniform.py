...
for setup in problem_list: # useless for loop at the moment, in case we want to run for different alpha variables
    ...
    file_name = "uniform_perishing_table_test_"+str(setup).replace('.','-')
    ...
    for n in num_groups:
        ...
        if setup == 'A':
            def perish_dist(b, n):
                ...
                return val
            low_range = np.asarray([n/2 - b/2 if b < (mean_size*n / 2) else n for b in range(max_budget)])
            up_range = np.asarray([n/2 + b/2 if b < (mean_size*n / 2) else n for b in range(max_budget)])
        ...
        for _ in tqdm(range(num_iterations)):
            ...
            for algo in algo_list:
                for alloc_order in order_list:
                    ...
                    if setup == 'A' and (alloc_order == 'lcb' or alloc_order == 'cv'):
                        x_lower_perish = x_lower_no_perish
                    elif setup == 'B' and (alloc_order == 'lcb' or alloc_order == 'mean'):
                        x_lower_perish = x_lower_no_perish
                    ...
                    data_dict = {'NumGroups': n, 'Algorithm': algo, 'Order': alloc_order, 'Norm': 'Swap', 'Value': swap}
                    data.append(data_dict)
